...
Node* remove_node_all_occurrences(Node* par_list, int par_num) {    // non chiede questo !!!!
	Node* cursor = par_list;
	Node* previous = NULL;
	Node* following;
	int found = 0;
	...
	while(cursor->next_node != NULL) {
		...
		if(cursor->num == par_num) {
			following = cursor->next_node;
			free(cursor);
			if(previous != NULL)
...
int node_exists(Node* par_list, int par_num) {
	int flag = 0;
	if(par_list == NULL){
		return flag;
	}
